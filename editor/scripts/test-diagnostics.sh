...
node -e '
  ...
  if (r.exit_code !== 1 || r.failure_class !== "content" || !r.used_stderr_fallback || !p) throw Error("stderr fallback mismatch");
  if (p.source_path !== "b.md" || p.line !== 1 || p.column !== 1 || p.origin !== "stderr" || p.position_confidence !== "best_effort") throw Error("stderr position mismatch");
