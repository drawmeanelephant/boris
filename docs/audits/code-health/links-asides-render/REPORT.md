...
| B5c | Indented fence, existing target, unbalanced | 3-space-indented fence + `[[fidelity-page]]` | exit 0; published code block contains the rewritten href `fidelity-page.html` (silent rewrite inside code) | **Confirmed defect → [#858]** | repro in #858 |
