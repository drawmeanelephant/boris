...
| Publication Touch Atlas | **Done on `afterparty`** — deterministic target-local `touches.json` relationship index derived exclusively from the committed bytes of `artifacts.json`, `checks.json`, and `claims.json`; runs after claims replacement with no payload rereads; OOM-safe construction. |
...
| 3 | Publication Touch Atlas | **Done on `afterparty`** | Derives and atomically replaces `touches.json` after claims commit; the contract's non-claims (source provenance, runtime traces, deployment graph, accessibility/prose inference, proof-pack, repairs) remain out of scope. |
