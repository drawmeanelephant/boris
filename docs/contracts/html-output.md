...
## Hosted publication locations

HTML remains target-relative by default: navigation, theme assets, and
content-local assets are emitted relative to the page that carries them. A
hosted build may supply the normalized Pages identity
`{base_url, origin, base_path, site_kind}` through `--pages-base-url`,
`--pages-origin`, and `--pages-base-path`. The pre-commit output gate then
requires that:

- a root-relative local URL includes the declared `base_path` for a project
...
This validates generated bytes in the local target transaction. It does not
fetch the deployed Pages site or claim post-deploy HTTP/browser success.
