...
**Product version field:** `boris_version` (package version string, currently `0.8.0`)
...
| `--split-size BYTES` | Deterministic byte cap for upload parts |
| `--bundles-only` | Emit upload parts/manifests and omit per-page files |
...
```json
{"format":"boris-rag","schema_version":1,"boris_version":"0.8.0"}
```
