...
| Cloudflare runtime/edge | **Runner example shipped** (#300); Wasm still open (#301) | Not a static rehost and not a registry member. `boris-job-runner` execs native `boris` behind an official Worker example ([`cloudflare-container-runner.md`](cloudflare-container-runner.md)). Freestanding Wasm embedding remains [#301](https://github.com/drawmeanelephant/boris/issues/301). These are compiler-shaped bets, not deployer adapters. |
