...
| Cloudflare runtime/edge | **Tracked separately, both with example hosts** (#300, #301) | Not a static rehost and not a registry member. Container-backed native builds behind a Worker (#300) have an official `boris-job-runner` example ([`cloudflare-container-runner.md`](cloudflare-container-runner.md)); freestanding Wasm embedding (#301) has an example Worker host at `hosts/cloudflare-worker/`. These are compiler-shaped bets, not deployer adapters, and live outside this matrix's deployer row. |
