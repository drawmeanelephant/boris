...
| Cloudflare runtime/edge | **Tracked separately** (#300, #301) | Not a static rehost: Container-backed native builds behind a Worker (#300) and freestanding Wasm embedding (#301). An example Worker host lives at `hosts/cloudflare-worker/`; it is not a deployer-matrix target. These are compiler-shaped bets, not deployer adapters, and live outside this matrix's deployer row. |
