...
- Cache-format/version discriminator (e.g., `boris-cache-v3-constant-digests`)
...
- On-disk `manifest.json` `format_version` must equal the fingerprint discriminator (currently `boris-cache-v3-constant-digests`). Manifests with any other version string are ignored (cold rebuild for that target).
