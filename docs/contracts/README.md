...
| Publication platform model | **Documented** — normative verified-target model, adapter seam, and platform deployment matrix; the target registry is `github-pages` and `standard-site`. GitHub Pages is the depth model; Standard.site is the second verified target. See [publication-platforms.md](publication-platforms.md). Pages-only wording remaining in [publication-profile.md](publication-profile.md) is filed drift ([#534](https://github.com/drawmeanelephant/boris/issues/534)), not the registry. |
