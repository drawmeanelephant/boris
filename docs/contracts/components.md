...
This is **not** generic HTML parsing, **not** MDX, and **not** a multi-component
plugin system. The only supported component is constrained **`<Aside>`**.
...
is not exactly `Aside` is a **hard error** (`unregistered_component` →
...
```text
segments → Apex(markdown) | aside.renderHtml(Aside) → ordered HTML body
```

No raw `<Aside>` tags in published HTML.
...
| `nested_component` | `ECOMPONENT` | Nested Aside |
...
- Valid Aside; optional id
- Invalid kind; duplicate attribute; unterminated quote
- Nested Aside; unknown component tag
- Aside-looking text inside fenced code remains literal
