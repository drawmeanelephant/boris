...
| Tree | What it proves |
|------|----------------|
| `content/` | Every accepted construct compiles: one-word and multi-word ingredients, quantities with and without units, a fraction-free and a unit-free amount, a short-hand preparation, named and anonymous timers, cookware, sections, a note, a forced line break, both comment forms, and a cross-recipe reference that must become a graph edge. It also carries a semantic relation, so the corpus exercises the IR 0.3 and 0.4 facets **together** — `relations` and `recipe` in one artifact, which is the combination that failed the published schema when `relations` reused the closed dependency-edge `kind` enum. |
