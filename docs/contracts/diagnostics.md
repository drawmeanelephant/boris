...
| `ENOSTRPLAN` | error | Plan: the corpus changed under the run — a selected source no longer parses after the graph validated. Sign: the input is not a valid plan/prior artifact (wrong format or schema), a `d` tag does not match its entity id, the intention digest does not match the plan bytes, or a prior bundle came from a different identity | `nostr_plan` / `nostr_sign` → `boris nostr plan` / `boris nostr sign` |
