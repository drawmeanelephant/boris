...
- The root `build.zig` reaches into `tools/` in exactly two places: the `source-rag` executable/test declaration, and `testdata-generator/src/generator.zig` as the shared `fixture_generator` module for publication fixture tests. Everything else under `tools/` is invisible to the root build graph except through the explicit `zig build --build-file` step for `test-github-pages-audit`.
- **Split-vs-keep**: the registry makes the seam visible. `search-index` and `github-pages-audit` are the only tools that import product modules — moving them out of tree would take their `src/*.zig` counterparts with them (both already keep that surface narrow). `docs-maintenance` and `source-rag` have no product coupling and could graduate without touching the product. `scale-smoke` and `testdata-generator` depend only on a built `boris` binary, so they move freely as long as the CLI contract holds.
- **Graduated**: `content-audit` left this repository for its own home ([`boris-content-audit`](https://github.com/drawmeanelephant/boris-content-audit), issue [#834](https://github.com/drawmeanelephant/boris/issues/834)); the migration laboratory left earlier ([`boris-migration-lab`](https://github.com/drawmeanelephant/boris-migration-lab)). A graduated tool keeps its own build, gate, and CI lane, and this registry covers the tools still in tree.
