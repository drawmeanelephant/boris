...
**As of:** 2026-07-15 · plan only · not a substitute for [`STATUS.md`](STATUS.md)  
**Assumption:** F8.1–F8.3 are fixed contract work in progress (or about to land).
Do not redesign F8. Plan everything *after* that ships, plus optional parallel
hygiene that avoids `pipeline` / `graph` / `json_out` while F8 is open.
...
**Today (product 0.2.1):** Boris is an HTML-first Zig documentation compiler —
Apex Unified, Trunk/Satellite graph, Feature 7 includes + wiki on the HTML path,
P2/P3 incremental / watch / jobs / multi-target. Machine IR still emits
`schemaVersion` **0.1.0** (parent-only edges, no reverse index).

**After Feature 8 (product 0.3.x):** the public IR becomes graph-native —
`schemaVersion` **0.2.0**, typed `page` / `source` endpoints, direct
`parent` / `include` / `reference` edges, deterministic `reverseIndex`, and
(via F8.3) a single frozen dependency story for incremental dirty sets. HTML
authoring and default CLI outcomes stay; the step-change is **one dependency
truth** for tools, rebuilds, and future edge kinds.

**Next ~3–6 cuts:** close F8 as **v0.3.0** (emit), unify dirty-set as **0.3.1**
if not bundled, docs hygiene, then build-system productization (**0.4**) and
authoring polish — not polyglot SSG work, not unrestricted MDX, not marketing
perf claims without measurement.
...
| Assumption | Detail |
|------------|--------|
| Binary ↔ IR | Claim IR 0.2 only when emit + goldens + compiler id `boris/0.3.0` ship. Current 0.2.1 must not advertise 0.2. |
...
- **Shipped:** v0.2.0 (HTML default, Apex, nav/TOC, P2/P3); v0.2.1 (Feature 7 includes/wiki).
- **Contracts ahead of binary:** F8.0 IR 0.2 contract frozen; fixture skeleton
  `docs/contracts/fixtures/graph-native-dependencies/`.
- **In flight:** F8.1 resolve+freeze · F8.2 emit+goldens+version · F8.3 reverse-index dirty-set.
- **Hardening:** adversarial issues #7–#28 closed; do not re-plan as greenfield.
- **STATUS drift (mild):** banner correctly says 0.2.1 + F8.0 contracted; “Active — F8.0”
  understates mid-flight F8.1+. RELEASE-GATE still titled v0.2.1 until F8.2 wires goldens.

### State after F8 lands (vs today)

After F8, consumers of `--out` get a frozen graph that matches include/wiki
...
rebuild?” behavior. Authors keep writing Markdown the same way; tooling and
large-site rebuild correctness improve.
...
|-------|---------|-------|--------------|
| Close F8 | **0.3.0** | F8.1 + F8.2 | `--out` emits IR 0.2 with edges + reverseIndex; compiler `boris/0.3.0` |
...
[today 0.2.1, IR 0.1]
        │
        ▼
   F8.1 freeze ──► F8.2 emit ──► tag v0.3.0 (IR 0.2)
        │                              │
        │                              ├─ Hygiene-G (docs)
        │                              ▼
...
### F8.1 + F8.2 → tag **v0.3.0** (in flight — do not redesign)
...
| **Why now** | F8.0 contract is normative; binary must catch up or consumers stay on 0.1.0 forever. |
| **Contract work** | Shape already in [`ir-schema.md`](contracts/ir-schema.md), [`includes-and-wiki-links.md`](contracts/includes-and-wiki-links.md); F8.2 promotes fixture skeleton → full goldens; RELEASE-GATE gains IR 0.2 checkbox. |
...
| **Verification** | `zig build test`; graph-native fixture golden; dual-run IR; `./scripts/release-gate.sh` after gate growth. |
...
**Tag policy:** **v0.3.0 = F8.1 + F8.2 complete**. Do not wait for F8.3 unless it is already green in the same freeze (then ship 0.3.0 with F8.3 and skip a thin 0.3.1).
...
| **Why now** | Contract notes F8.3 may use the frozen pair; HTML already has fingerprints + `getAffectedPages`; unify rather than grow a second graph. |
...
|-------|---------|
| **Name / id** | Hygiene-G |
| **User outcome** | Agents and humans read one story: product 0.3.x, IR 0.2, next work not “F8.0 active.” |
...
| **Risks** | Overclaiming F8.3 if only F8.2 tagged. |
...
**Week 2 — Tag v0.3.0 + Hygiene-G**

- Merge F8.2; cut **v0.3.0**.
...
1. **docs(hygiene):** STATUS / README / RELEASE-GATE / CHANGELOG for 0.3.0 (if not already in the tag PR).
2. **feat(f8.3):** reverseIndex-backed `getAffectedPages` + incremental e2e; tag **0.3.1**.
3. **test/chore(p4-cache):** residual freshness tests; document honesty limits.
4. **docs/feat(p4-watch):** contract note or selective fan-out only with tests.
5. **feat(f9):** content dogfood / TOC; heading wiki only with a prior contract PR.
