...
The file has no dependency on the Markdown renderer for its primary IR pipeline path. Its module-level comment explicitly states "No HTML, layouts, or Markdown rendering on this path." The Oliver-backed rendering seam enters only through transitive compilation of `aside.zig`, which is imported via `aside.tokenizeBody`. Because `aside.zig` renders through `render_mod`, `pipeline.zig` must also be linked with `render_mod` at build time, but `pipeline.zig` itself never calls any renderer function directly. This distinction is structurally important: the render-seam link is a build dependency, not a behavioral one, for the IR pipeline.
...
Three public JSON-rendering functions (`renderManifest`, `renderGraph`, `renderBuildReport`) delegate entirely to `src/ir_emit.zig`, passing a version-constant bundle. Version constants are declared at the top of the file: `schema_version = "0.2.0"`, `compiler_id = "boris/0.8.1"`, `semantic_schema_version = "0.3.0"`, `semantic_compiler_id = "boris/0.8.1+semantic-relations"`, and `boris_version = "0.8.1"`. These are the authoritative source of version identity embedded in every build output.
...
The file does not test anything related to the rendering seam, HTML rendering, RAG export, or incremental/HMR paths. Those concerns belong to `src/render.zig`, `src/rag.zig`, and related modules.
...
`src/pipeline.zig` sits one layer above all individual subsystem modules and one layer below the CLI entry point (`src/main.zig`) and the RAG/package tools. It is linked into the product binary: `build.zig` creates `pipeline_mod` from `src/pipeline.zig`, calls `linkOliver(pipeline_mod, oliver_mod)`, and adds it to the `test` step via `run_pipeline_tests`. The same `pipeline_mod` import chain feeds `root_mod` (the product CLI).

The pipeline module does **not** render Markdown itself. The render-module link is required only because `src/aside.zig` is `@import`-ed, and `aside.zig` in turn calls `render.render` through `render_mod`. Removing the `linkOliver` call from `pipeline_mod` would produce a link error even though no renderer function is reachable from `pipeline.zig`'s own code paths in the IR build mode.

Markdown rendering is owned by `src/render.zig` (the Oliver-backed seam), which `pipeline.zig` reaches transitively through `html_body.zig` and `aside.zig`. There is no C engine and no hostile double.
