...
This file has no relationship to the Oliver-backed rendering seam (`src/render.zig`). It does not import `render.zig` and does not appear in any renderer wiring in `build.zig`.

The file's inline tests (`zig build test`) run inside the `root_mod` test binary, which is also linked against the Oliver-backed rendering seam via `linkOliver(root_mod, oliver_mod)`, but none of the tests in `html_nav.zig` call any renderer function. The renderer link is present because `root_mod` aggregates all product imports transitively; it is not an architectural coupling.
