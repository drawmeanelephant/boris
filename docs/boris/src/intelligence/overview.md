...
`src/intelligence.zig` is a **leaf module** in the Boris dependency graph. It imports only `std` and defines all its types locally. It does not import `src/render.zig`, `src/graph.zig`, `src/pipeline.zig`, or any other Boris module, and `build.zig` confirms this: the `intelligence_mod` is created with only `root_source_file` and no `imports` array, and no renderer module is linked for it. This makes it entirely independent of the rendering seam and safe to compile and test on any host.
...
The rendering seam (`src/render.zig`) is entirely separate; `src/intelligence.zig` has no interaction with it.
