...
This file sits entirely outside the production binary. The `build.zig` `layout_hostile_mod` module is created with `src/layout_select_hostile_test.zig` as its root and depends on `render_mod`, but it is not linked into `exe` or any other installed artifact. The test binary is ephemeral, created only when `zig build test` or `zig build test-layout-hostile` is invoked.
...
This file is "hostile" in the sense of adversarial *usage patterns* — ambiguous globs, traversal paths, permuted rule tables, missing files — aimed at the layout selection contracts, not at the renderer. It exercises full compile rounds through the Oliver-backed rendering seam (`src/render.zig`); Oliver is a pure Zig library with no C ABI and no hostile double.
