...
There is no connection to `src/render.zig` or the Oliver-backed rendering seam. `layout_select.zig` is a pure Zig module tested against adversarial *usage patterns* (ambiguous globs, traversal paths, permuted rule tables, missing files). Its companion test file, `src/layout_select_hostile_test.zig`, provides integration-level coverage by running full compile rounds.
