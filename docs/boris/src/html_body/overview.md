...
The file exists because the body-rendering logic was previously duplicated between at least two call sites (`compile.zig`'s HTML publish path and the heading-index path). Extracting it into a shared module enforces a single, documented ordering contract — parse/adapt → include expansion → wiki-link rewrite → content-local asset rewrite → Aside tokenization → Oliver/Aside streaming — and eliminates drift between paths that must behave identically.

The system boundary this module protects is the contract between Boris's Zig pipeline stages and the Oliver rendering seam (`src/render.zig`). Everything upstream of the `render.render` call happens in pure Zig and is fully inspectable; the module never calls `render.render` on the whole document, only on Markdown-typed segments after Aside tokenization has split the body. This ensures Aside component markup is never passed to the renderer, and Oliver is only given clean, pre-validated Markdown slices.
...
| Main collaborators | `src/render.zig`, `src/aside.zig`, `src/parser.zig`, `src/include.zig`, `src/wikilink.zig`, `src/content_asset.zig`, `src/textile.zig`, `src/diag.zig`, `src/graph.zig`, `src/identity.zig` |
...
Relative to the product binary (`src/main.zig` → `compile.zig` → `html_body.zig`), the module is a production dependency compiled into every `boris build` invocation whenever an HTML output pass is triggered. It is not gated by any build option or feature flag; `linkOliver` applies to the product root, so the Oliver-backed seam is always present.

Relative to `src/render.zig`, `html_body.zig` is a consumer. It does not import `render.zig`'s internals; it calls the public `render.render(md, &doc_arena)` function for each `.markdown` segment. The arena lifetime contract is satisfied because `html_body.zig` holds `doc_arena` alive for the duration of `renderSource` and returns a slice into that same arena.

Oliver is a pure Zig library with no C engine and no hostile double; `html_body.zig` uses the same `render.render` seam in tests as in production. The module's tests are part of the main module test binary and render through the pinned Oliver dependency exactly as the CLI does.
