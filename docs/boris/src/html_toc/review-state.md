...
- **Unclosed inner tags:** `stripTags` silently drops everything from an unclosed `<` to the end of inner content. This is structural behavior (not a contract violation for Apex-produced HTML) but is not tested. The result is a truncated `text` value rather than an error.
