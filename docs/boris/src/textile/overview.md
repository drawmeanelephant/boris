...
`src/textile.zig` is a **pipeline pre-stage adapter**: it executes before the renderer is ever called. Its position in the documented pipeline is:
...
scan selected .textile extension
  ...
  → Oliver render → IR / RAG / Context / layout splice
```

The module is not a test-only file. It is a library that the production binary depends on whenever `--textile` mode is active. It does not call `src/render.zig` at any point; Oliver only ever sees the produced Markdown, not the Textile source. Rendering happens downstream and is entirely independent of this module.

The module has no relationship to the rendering seam (`src/render.zig`). Its only non-`std` import is `parser.zig`, and that import appears only inside an `expectFixture` test helper — it is not required at library call sites.

The normal test suite includes this module's tests in `zig build test`. There is no separate opt-in step required.
