...
- **ASan/UBSan coverage:** The renderer is pure Zig (Oliver), so there is no C adapter to sanitize; allocator misuse in pure-Zig code relies on `std.testing.allocator`'s leak detection in test mode.
...
- A sanitizer pass over the Zig-only identity logic (e.g., via `zig build test -Doptimize=Debug` with `std.testing.allocator`) could confirm leak-freedom under all error branches.
