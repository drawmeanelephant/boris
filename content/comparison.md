...
| **Build Speed** | N/A | Seconds to minutes | **Sub-second native execution**[^speed] |
| **Memory Model** | N/A | Garbage collected JS heap | **Per-page arena scratch allocation**[^memory] |
| **Link & Graph Safety** | None | Optional plugins | **Strict Fail-Loud Graph Validation** |
...
| **Client JS Footprint** | None | Hydration JS bundles | **Zero Required Client JS** |
...
[^speed]: Sub-second build times are measured on standard developer hardware rendering 1,000+ Markdown pages through in-process C ABI calls without subprocess spawning or bundler overhead.
[^memory]: Per-page arena allocation ensures that memory scratch space allocated while parsing a page is freed immediately upon page reset, keeping total RSS flat throughout long build runs.
