...
## Zig documentation tasks

For tasks whose primary purpose is reviewing or writing Zig documentation,
read and follow [`docs/agent-policies/zig-documentation.md`](docs/agent-policies/zig-documentation.md)
before planning or editing.

Documentation-only tasks must not change runtime behavior, declarations,
signatures, imports, build configuration, or tests unless explicitly authorized.

Derive documentation from implementation, tests, call sites, and repository
contracts. Report uncertainty rather than guessing.

Keep documentation work in small subsystem-based batches, validate formatting,
builds, and tests, and inspect the final diff for accidental code changes.
